...
module "elasticsearch" {
  source                  = "git::https://github.com/cloudposse/terraform-aws-elasticsearch.git?ref=0.20.2"
...
resource "aws_cloudwatch_log_group" "application_group" {
  name = "/aws/aes/domains/procurify-production-logging/application-logs"
}

resource "aws_cloudwatch_log_group" "index_group" {
  name = "/aws/aes/domains/procurify-production-logging/index-logs"
}

resource "aws_cloudwatch_log_group" "search_group" {
  name = "/aws/aes/domains/procurify-production-logging/search-logs"
}
