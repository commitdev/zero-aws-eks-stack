# Create/Update mysql db user with kubernetes job
...
resource "kubernetes_secret" "db_create_users" {
  ...
  data = {
    "create-user.sql" = <<-EOF
      ...
      CREATE USER ${var.db_app_user} IDENTIFIED BY "${var.db_app_password}";
      GRANT ALL PRIVILEGES ON ${var.db_name}.* TO ${var.db_app_user};
EOF
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["sh"]
          args    = ["-c", "mysql -u${var.db_master_user} -h database.${var.namespace} ${var.db_name} < /db-ops/create-user.sql"]
